...
class SoPmi:
    ...
    def collect_candiwords(self, seg_data, cowords_list, sentiment_path):
        ...
        '''收集种子情感词'''
        def collect_sentiwords(sentiment_path):
            pos_words = set([line.strip().split('\t')[0] for line in open(sentiment_path).readlines()
                             if float(line.strip().split('\t')[1]) > 0])
            neg_words = set([line.strip().split('\t')[0] for line in open(sentiment_path).readlines()
                             if float(line.strip().split('\t')[1]) < 0])
        ...
        pos_words, neg_words = collect_sentiwords(sentiment_path)
        pmi_dict = compute_sopmi(candi_words, pos_words, neg_words, word_dict, co_dict, all)
        return pmi_dict

    def sopmi(self):
        cowords_list = self.collect_cowords(self.sentiment_path, self.seg_data)
        pmi_dict = self.collect_candiwords(self.seg_data, cowords_list, self.sentiment_path)
        # print(pmi_dict)
